...
- 入力して正誤を判定する「クイズモード」と、答えを見ながら学ぶ「確認モード」
...
Web Speech APIに対応していないブラウザでは発音ボタンが無効になり、クイズ・確認モード・単語一覧のその他の機能はそのまま利用できます。外部の音声ファイルや有料APIは使用していません。

## 学習モード

クイズモードでは、表示された拼音に対応する簡体字を入力して正誤を判定します。確認モードでは入力や正誤判定を行わず、「答えを見る」を押して中国語と日本語の意味を確認します。

確認モードを最後まで進めると確認完了画面、途中で終了すると確認済み単語だけの確認途中結果を表示します。確認済みの単語群から入力式クイズを開始する場合は、問題を再度シャッフルして出題します。

確認モードでは正誤を記録しないため、間違い復習用の `localStorage` は変更しません。「間違えた問題だけ復習」は、ホームで選択中の学習モードにかかわらず、従来どおり入力式クイズとして開始します。
...
サイト側からキーボードを自動的に「簡体中国語・手書き」へ切り替えることはできません。クイズへ解答するときは、ユーザー自身で端末のキーボードを「簡体中国語・手書き」に切り替えてください。
...
出題数の「全問」は全77語固定ではなく、現在選択している課に含まれる全単語を意味します。指定した出題数が候補語数を超える場合は、重複を作らず候補語をすべて出題します。課選択はクイズモードと確認モードの両方に適用され、途中で「最初からやり直す」を選んでも開始時点の課が維持されます。
...
- `script.js`: 画面切り替え、課選択、クイズ・確認モードの進行、履歴、検索、保存処理
